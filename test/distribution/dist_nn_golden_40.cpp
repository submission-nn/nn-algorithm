...
#define G_n                     40
constexpr uint64_t gam = 10;    // need to use a different name. Apparently 'gamma' is a name of function in the std
...
TEST(Dist_GoldenNearestNeighbor_40, Quadratic) {
	uint64_t pos1, pos2;
	NNList L1{1}, L2{1};
...
TEST(Dist_GoldenNearestNeighbor_40_10, Windowed) {
	...
	run_dist(gam, w , d, r, N, size, find_all, THRESHHOLD, 0, "w:40:0:10");
}

TEST(Dist_GoldenNearestNeighbor_40_10, WindowedWithEpsilon) {
	uint64_t pos1, pos2;
	NNList L1{1}, L2{1};
	...
	run_dist(gam, w , d, r, N, size, find_all, THRESHHOLD, epsilon, "w:40:1:10");

}

/* NOT WORKIN
TEST(Dist_GoldenNearestNeighbor_40_15, Windowed) {
	// Laut Script für n = 40 \lambda = 0.25 da size = 10
...
TEST(Dist_GoldenNearestNeighbor_40_15, WindowedWithEpsilon) {
	uint64_t pos1, pos2;
	NNList L1{1}, L2{1};
